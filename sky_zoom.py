...
def update(frame):
    ...
    ax.set_xlim(pos_np[frame, -1, 0] - 2*pos_np[:, -1, 3].max(), pos_np[frame, -1, 0] + 2*pos_np[:, -1, 3].max())
    ax.set_ylim(pos_np[frame,-1, 1]  - 2*pos_np[:, -1, 3].max(), pos_np[frame, -1, 1] + 2*pos_np[:, -1, 3].max())
...
ani.save("sky_zoom.mp4", writer="ffmpeg", fps=30)